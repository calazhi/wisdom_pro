      ...
      <el-form :inline="true" :model="searchConditionList">
        <div class="search-container">
          <el-form-item label="配置项名称">
            <el-input class="appupgrade_el-select" placeholder="请输入配置项名称" v-model.trim="searchConditionList.codeName" @keyup.enter.native="_handleFilter"> </el-input>
          </el-form-item>
          <el-form-item label="代码值" :label-width="formLabelWidth">
            <el-input class="appupgrade_el-select" placeholder="请输入配置项的值" v-model.trim="searchConditionList.code" @keyup.enter.native="_handleFilter"> </el-input>
          </el-form-item>
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="代码类别" :label-width="formLabelWidth" prop="codeTypeUuid">
                <el-select v-model="registerParaList.codeTypeUuid" placeholder="请选择代码类别" style="width:320px;" clearable  :disabled="disabled" >
        ...
        .then(
          function (result) {
            ...
            for (let i = 0; i < codeCategoryArr.length; i++) {
              if (codeCategoryArr[i].typeName === '配置类别代码') {
                this.codeCategories.push({
                  label: codeCategoryArr[i].typeName,
                  value: codeCategoryArr[i].uuid
                })
                this.registerParaList.codeTypeUuid = this.codeCategories[0].value
              }
            }
          }.bind(this)
        )
