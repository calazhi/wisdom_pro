<template>
  <div class="ui-common"> 
    ...
    <!--软件包批量导入 -->
    <div>
      <el-dialog :title="dialogImportTitle" :visible.sync="dialogImportVisible" :before-close="closeImportDialog" v-loading="uploadloading">
        <div>
          <el-form>
            <el-row>
              <el-col :span="36">
              ...
              <el-col :span="36">
                <el-form-item label="大文件软件包以及关联附件" :label-width="formLabelWidth">
                  <el-upload
                    class="avatar-uploader"
                    :action="fileUploadUrl"
                    :on-success="handleUpLoadResponse"
                    ...
                    :file-list="fileJarAttachmentList">
                    <i class="el-icon-upload"></i>
                    <div class="el-upload__text">将上传大文件附件拖到此处，或<em>点击上传</em></div>
                  </el-upload>
                </el-form-item> 
              </el-col>
            </el-row>
...
import { getBatchesList, getSoftwarePackageByPage, getsoftwarePckById, deleteSoftwarePack, updateSoftwarePackage, registerSoftwarePackage, downCfgMgmtTemplate, uploadExcelFiles, getFileUploadUrl } from './apis/index'
export default {
  components: {
  ...
  methods: {
    ...
    _registerSoftware (formName) {
      ...
      for (let i = 0; i < this.fileNameArray.length; i++) {
        this.uploadFiles.append('fileNames', this.fileNameArray[i])
      }
      for (let i = 0; i < this.urlArray.length; i++) {
        this.uploadFiles.append('urls', this.urlArray[i])
      }
      console.info('_registerSoftware add')
      console.info(this.uploadFiles)
      var fileLength = this.fileNameArray.length
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (fileLength > 0) {
            ...
            .then((res) => {
              console.log('===>' + res)
              this.$message.success('上传成功', 5000)
    ...
    _importRegisterSoftware () {
      ...
      console.info(this.fileNameArray.fileLength)
      // var fileJarAttachmentLength = this.$refs.uploadPackageJarAttachmentFiles._data.uploadFiles.length
      if (fileLength > 0 && this.fileNameArray.length > 0) {
        this.importFiles = new FormData()
        this.importFiles.append('excelfile', this.excelArray[0])
        for (let i = 0; i < this.fileNameArray.length; i++) {
          this.importFiles.append('fileNames', this.fileNameArray[i])
        }
        for (let i = 0; i < this.urlArray.length; i++) {
          this.importFiles.append('urls', this.urlArray[i])
        }
    ...
    handleUpLoadResponse (response) {
      console.info('append handleUpLoadResponse')
      this.fileNameArray.push(response.fileName)
      this.urlArray.push(response.uri)
      console.info(this.fileNameArray)
      console.info(this.urlArray)
    },
    handleUpLoadremove (file, fileList) {
      console.info('append handleUpLoadremove')
      for (let i = 0; i < this.fileNameArray.length; i++) {
        if (this.fileNameArray[i] === file.name) {
          this.fileNameArray.splice(i, 1)
        }
        if (this.urlArray[i].indexOf(file.name) !== -1) {
  ...
  mounted () {
    ...
    getFileUploadUrl('file-upload-url')
      .then(
          function (result) {
            console.log(result)
            console.log('<<<<<getFileUploadUrl:')
            this.fileUploadUrl = result.data
