...
export const uploadExcelFiles = (params) => {
  ...
  // params
  return Axios.post(BASE_PATH + '/package/importPackageExcelTemplateNew', params, config
  ).then(res => {
...
export const registerSoftwarePackage = (params1, params2) => {
  let contentType = {
    headers: {'Content-Type': 'multipart/form-data;text/html;charset=UTF-8'}
  }
  console.log('<<<<< software register  params:' + JSON.stringify(params1) + ' =============== > ' + params2)

  return Axios.post(BASE_PATH + '/package/createNew?' + 'batchesId=' + encodeURI(params1.batchId) + '&name=' + encodeURI(params1.name) + '&version=' + encodeURI(params1.version) + '&provider=' + encodeURI(params1.provider) +
  '&latestPreVer=' + encodeURI(params1.latestPreVer) + '&lastestPreName=' + encodeURI(params1.lastestPreName) + '&softwareName=' + encodeURI(params1.softwareName) + '&softwareNameEn=' + encodeURI(params1.softwareNameEn) + '&functionDesc=' + encodeURI(params1.functionDesc) +
  '&remark=' + encodeURI(params1.remark) + '&nameEn=' + encodeURI(params1.nameEn) + '&gitRepository=' + encodeURI(params1.gitRepository) + '&mavenName=' + encodeURI(params1.mavenName) +
  '&svrPkgName=' + encodeURI(params1.svrPkgName) + '&cltPkgName=' + encodeURI(params1.cltPkgName) + '&cltPkgName=' + encodeURI(params1.cltPkgName) + '&port=' + encodeURI(params1.port) +
  '&functionType=' + encodeURI(params1.functionType) + '&appType=' + encodeURI(params1.appType),
...
// 获取大文件上传路径
export const getFileUploadUrl = (key) => {
  console.log('get fileuploadUrl ---- >' + key)
  return Axios.post(BASE_PATH + '/configs/findByItem?itemKey=' + key).then(res => res.data)
}
