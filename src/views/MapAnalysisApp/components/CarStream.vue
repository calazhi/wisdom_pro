      ...
      <el-row>
        <el-col :span="4">
          <div class="description">
            <div>
              <p>小区名称</p>：{{courtInfo.name}}
            </div>
            <div>
              <p>小区地址</p>：{{courtInfo.address}}
            </div>
            <div>
              <p>户 数</p>：{{courtInfo.homeCount}}户
            </div>
            <div>
              <p>房屋总数</p>：{{courtInfo.houseCount}}栋
            </div>
            <div>
        ...
        <el-col :span="20">
          ...
          <div class="data-view">
            <div class="car-info-table" v-show="isShowTable">
              <!-- 展示表格开始 -->
              <el-table stripe :data="carStreamData" height="400" border style="width: 100%" v-loading="loading" element-loading-text="拼命加载中" element-loading-background="rgba(255, 255, 255, 1)">
...
export default {
  ...
  data () {
    return {
      disabled: true, // 时间选择器禁用,解决时间选择器类型切换时bug
      ...
      preEchartsFormState: null, // echarts图表查询时的页面form的上一次状态
      preTableFormState: null, // table图表查询时的页面form的上一次状态
      ...
      forbiddenEndDatetime: { // 限制结束时间选择器
        disabledDate: (time) => {
          return time.getTime() > Date.now() || time.getTime() < this.form.startTime
        }
      }
    }
  },
  mounted: function () {
    this.form.startTime = new Date(new Date(new Date().setDate(new Date().getDate() - 6)).setHours(0, 0, 0, 0))
    this.form.endTime = new Date(new Date().setHours(0, 0, 0, 0))
  },
  methods: {
    /**
    ...
    */
    onReportTypeSelected: function () {
      if (this.form.reportType === '0') {
        ...
        this.formDatePickType = 'date'
        this.form.startTime = new Date(new Date(new Date() - 86400000 * 30).setHours(0, 0, 0, 0))
        this.form.endTime = new Date(new Date().setHours(0, 0, 0, 0))
      } else {
        this.formDatePickType = 'month'
        this.form.startTime = new Date(new Date(new Date() - 86400000 * 30 * 12).setHours(0, 0, 0, 0))// 开始时间
        this.form.endTime = new Date(new Date().setHours(0, 0, 0, 0))
      }
    },
    /**
    ...
    */
    onHandleCurrentChange: function (val) {
      ...
      if (this.onDatePickRangeConfrim()) {
        this.form.startTime = new Date(this.queryParam.startTime.replace(/-/g, '/'))
        this.form.endTime = new Date(this.queryParam.endTime.replace(/-/g, '/'))
      }
      this.getCarAccessPageList()
    },
    /**
    ...
    */
    getCourtCarAccessInfo: function () {
      if (this.onDatePickRangeConfrim()) return
      this.chartInit()
      if (JSON.stringify(this.form) === this.preEchartsFormState) return
      ...
      getCourtCarAccessInfo(data).then((res) => {
        if (res.data.code === '00000') {
          ...
          if (res.data.data.length && res.data.data.length > 0) {
            responedata = res.data.data
          } else {
            var time = this.timeFomate(this.form.endTime)
            responedata = [{ date: time, carInCount: '0', carOutCount: '0', carInRegedCount: '0', carOutRegedCount: '0' }]
          }
          this.sortData(responedata)
          ...
          // 注册图表缩放控件事件
          var zoomSize = 8
          this.disPatchAction(this.myChart, this.mapDataList.date, { dataIndex: 0 }, zoomSize)
          this.myChart.off('click')
          this.myChart.on('click', function (params) {
            this.disPatchAction(this.myChart, this.mapDataList.date, params, zoomSize)
          }.bind(this))
          this.preEchartsFormState = JSON.stringify(this.form)
    ...
    */
    getCarAccessPageList: function () {
      if (this.onDatePickRangeConfrim()) return
      this.carStreamData = JSON.parse(JSON.stringify(this.carStreamData))
      this.totalRows = JSON.parse(JSON.stringify(this.totalRows))
      if (JSON.stringify({ currentPage: this.currentPage, pageSize: this.pageSize, ...this.form }) === this.preTableFormState) return
      this.loading = true
      ...
      getCarAccessPageList(this.queryParam).then(res => {
        if (res.data.code === '00000') {
          ...
          this.preTableFormState = JSON.stringify({ currentPage: this.currentPage, pageSize: this.pageSize, ...this.form })
        }
        this.loading = false
      }).catch(() => {
        this.loading = false
      ...
      return {
        ...
        toolbox: {
          ...
          feature: {
            ...
            saveAsImage: { show: true, right: '20' }
          }
        },
        calculable: true,
        xAxis: [
        ...
        dataZoom: [
          {
            ...
            xAxisIndex: [0],
            start: 0,
            end: this.mapDataList.date.length > 15 ? 25 : 100
          },
          {
            type: 'inside',
            show: true,
