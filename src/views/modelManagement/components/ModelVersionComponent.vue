        ...
        <el-col :span="4">
          <div>
            <el-form :inline="true" :model="formInline" class="demo-form-inline">
              &nbsp;
              <el-form-item label="">
                <el-button v-show="isStructuredNode" :disabled="loadingVersion" @click="showAddModelDialog()" type="primary" icon="el-icon-circle-plus-outline">添加版本</el-button>
              </el-form-item>
            </el-form>
          </div>
        </el-col>
          ...
          <template slot-scope="scope">
            ...
            <el-dropdown @command="handleCommand($event, scope.$index, scope.row)" trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-if="isStructuredNode && scope.row.fileUploadFlag && scope.row.versionStatus!='mm.versts.disable'" class="model-dropdown-item" command="deploy"><i class="el-icon-tickets"></i>部署发布</el-dropdown-item>
                ...
                <el-dropdown-item v-if="isStructuredNode && scope.row.deployed && scope.row.versionStatus!='mm.versts.disable'" class="model-dropdown-item" command="excuteTask"><i class="el-icon-tickets"></i>调用模型</el-dropdown-item>
                <el-dropdown-item v-if="isStructuredNode && scope.row.deployed" class="model-dropdown-item" command="taskplan"><i class="el-icon-time"></i>查看计划</el-dropdown-item>
                <el-dropdown-item v-if="isStructuredNode && scope.row.deployed" class="model-dropdown-item" command="task"><i class="el-icon-search"></i>查看任务</el-dropdown-item>
                ...
                <el-dropdown-item v-if="isStructuredNode && scope.row.fileUploadFlag && scope.row.versionStatus!='mm.versts.disable' && !scope.row.deployed" class="model-dropdown-item" command="updatefile"><i class="el-icon-upload"></i>更新模型文件</el-dropdown-item>
                <el-dropdown-item v-if="isStructuredNode && !scope.row.fileUploadFlag" class="model-dropdown-item" command="updatefile"><i class="el-icon-upload"></i>上传算法文件</el-dropdown-item>
                ...
                <el-dropdown-item v-if="isStructuredNode" class="model-dropdown-item" command="edit" divided><i class="el-icon-edit-outline"></i>修改</el-dropdown-item>
                <el-dropdown-item v-if="isStructuredNode && !scope.row.deployed" class="model-dropdown-item" command="delete" ><i class="el-icon-delete"></i>删除</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
    ...
    <el-dialog
      ...
      <div class="text-center">
        ...
        >
          ...
          <div v-if="!isStructuredNode" class="el-upload__tip" slot="tip"><span v-if="showLimitSize">仅支持</span>上传<span v-if="showLimitSize">不超过5G的</span>视频分析程序</div>
          <div v-if="isStructuredNode" class="el-upload__tip" slot="tip"><span v-if="showLimitSize">仅支持</span>上传<span v-if="showLimitSize">不超过50M的</span>结构化分析程序</div>
      ...
      <el-form v-loading="loadingExcuteTask" :model="excuteVersionTask" ref="excuteVersionTask" label-width="40%" class="demo-ruleForm">
        ...
        <div v-if="currentTaskExeStep == 2" class="excute-min-height">
          ...
          <div style="height: 14px; line-height: 22px; padding: 0 0 0 15px;" class="red">
            ...
            <div v-if="isExeEmptyNode && currentDeployPublishAction==='excute'">请选择要调用模型的小区</div>
          </div>
        </div>
        ...
        <div class="add-model-button" v-if="currentTaskExeStep == 2">
          <el-row :gutter="24">
            ...
            <el-col :span="18" class="text-right">
              ...
              <el-button type="primary" @click="startToexcuteVersionTask()">调用模型</el-button>
            </el-col>
          </el-row>
...
<script>
  import {
    ...
    addModelVersion,
    getUploadSystemSetting,
    uploadVersionFileInfo,
  ...
  export default {
    ...
    data () {
      return {
        ...
        multipleSelection: [],
        deployCommand: {},
        currentUploadVersionId: 0,
        showLimitSize: LIMIT_FILE_SIZE_FLAG,
        excuteCommunityList: [],
        communityStatusList: [],
    ...
    methods: {
      ...
      afterUploadSendMessage (res) {
        console.info(res)
        const params = {
          algVersPk: this.currentUploadVersionId,
          fileName: res.fileName ? res.fileName : 'demo.zip',
          filePath: res.uri ? res.uri : '/model/demo.zip',
          fileSize: res.fileSize
        }
        let loadingMessage = startSystemLoading()
        uploadVersionFileInfo(params)
        .then(
            function (result) {
              this.$nextTick(() => {
      ...
      uploadSucess (response, file, fileList) {
        ...
        // this.loadData()
        // this.uploadDialogVisible = false
        this.afterUploadSendMessage(response)
      },
      ...
      uploadModelFile (id) {
        ...
        this.currentUploadVersionId = id
        let loadingSettings = startSystemLoading()
        getUploadSystemSetting()
        .then(
            function (result) {
              console.log(result)
              this.uploadParam.url = UPLOAD_PROTOCOL + result.data.host + ':' + result.data.uploadPort + UPLOAD_SUFFIX
              this.uploadParam.data = {
                module: result.data.contextModule
              }
              this.$nextTick(() => {
      ...
      handleCommand (event, index, item) {
        if (event === 'edit') {
          ...
          // this.showUploadArea = true
          // this.uploadDialogVisible = true
          this.uploadModelFile(this.modelVersionList[index].algModelVersionPk)
