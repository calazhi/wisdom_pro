            ...
            <div class="margin-bottom-15">
              <template v-for="(param, index) in curParamList">
                <el-form-item :label="param.paramDesc" prop="paramValue">
                  ...
                  <el-input placeholder="" v-model="param.paramValue">
                    <template v-if="param.paramName == 'mm.restyp.logpath'|| param.paramName == 'mm.restyp.filepath'" slot="prepend">{{pathPrefix}}</template>
                  </el-input>
                </el-form-item>
              </template>
            </div>
    ...
    <el-dialog
      ...
      <el-form v-loading="loadingEditStep" :model="editModel" :rules="rules" ref="editModel" label-width="150px" class="demo-ruleForm">
        ...
        <el-form-item label="参数值" prop="paramValue">
          ...
          <el-input placeholder="" v-model="editModel.paramValue">
            <template v-if="editModel.paramName == 'mm.restyp.logpath'|| editModel.paramName == 'mm.restyp.filepath'" slot="prepend">{{pathPrefix}}</template>
          </el-input>
        </el-form-item>
...
<script>
  ...
  import { getResultByVersionID, getResTypeSubsetMetaCats, saveAlgVersResult, getResultFilePrefixByVersionId } from '@/views/modelManagement/apis/version_resultdefine_api'
  ...
  export default {
    ...
    methods: {
      ...
      getPathPrefix (type, item, index) {
        const param = {
          versionId: this.algModelVersionPk
        }
        getResultFilePrefixByVersionId(param)
          .then(
            function (result) {
              this.pathPrefix = result.data
              console.info(result)
              if (type === 'add') {
                ...
                console.info(item)
                this.editModel = item
                if (this.editModel.paramName === 'mm.restyp.filepath' || this.editModel.paramName === 'mm.restyp.logpath') {
                  let tmpSplitArr = this.editModel.paramValue.split(this.pathPrefix)
                  if (tmpSplitArr && tmpSplitArr.length > 1) {
                    this.editModel.paramValue = tmpSplitArr[1]
                  }
                  // this.editModel.paramValue = this.editModel.paramValue.split(this.pathPrefix)
                }
                this.curIndex = index
      ...
      handleEdit (index, item) {
        ...
        // this.editResultDialogVisible = true
        this.getPathPrefix('edit', this.editModel, this.curIndex)
      },
      // 根据结果类型获取结果参数列表
      ...
      confirmAddAlgModel () {
        ...
        params.params.forEach(function (element, index) {
          if (element.paramName === 'mm.restyp.filepath' || element.paramName === 'mm.restyp.logpath') {
            element.paramValue = this.pathPrefix + element.paramValue
          }
        }.bind(this))
        console.log(params)
      ...
      confirmEditAlgModel () {
        ...
        params.resultType = this.resultType
        if (this.editModel.paramName === 'mm.restyp.filepath' || this.editModel.paramName === 'mm.restyp.logpath') {
          this.editModel.paramValue = this.pathPrefix + this.editModel.paramValue
        }
