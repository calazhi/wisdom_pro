    ...
    submitForm (formName) {
      ...
      self.$refs[formName].validate(valid => {
        if (valid) {
          ...
          console.log(params)
          console.log('next login() submitForm self.isBtnLoading:' + self.isBtnLoading)
          login(params).then(result => {
            console.log(' login(params) self.isBtnLoading:' + self.isBtnLoading)
            self.isBtnLoading = false
            console.log('22222222' + JSON.stringify(result))
            let homePath = '/home'
            if (result.data.token) {
              localStorage.setItem('login_username', self.ruleForm.username)
              ...
              let egcUserStr = JSON.stringify(result.data)
              console.log('egcUserStr begin')
              console.log(egcUserStr)
              console.log('egcUserStr end')

              let meuns = JSON.stringify(result.data.uiResources)
              console.log(meuns)
              localStorage.setItem('meuns', meuns)
              ...
              localStorage.setItem('userRouters', JSON.stringify(userRouters))
              localStorage.setItem('userResourcePermission', JSON.stringify(userResourcePermission))

              //  no login 20180118
            } else {
              self.isBtnLoading = false
              console.log('登录不成功，请重试。。。')
