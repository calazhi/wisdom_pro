<template>
  <el-dialog :visible.sync='deviceCategoryDetailVisible' :modal-append-to-body='false' :before-close='closeDialog' style="min-width: 750px;" :close-on-click-modal="false">
        ...
        <div style="padding-left: 30px">
          ...
          <el-form :model='deviceCategoryDetail' ref='deviceCategoryDetail' label-width='160px' :rules='rules' :inline='true'>
            <el-form-item label='设备主数据编码' prop='typeCode'>
              <el-select v-if="!viewFlag" v-model='deviceCategoryDetail.typeCode' @change="typeCodeChange($event)" filterable>
...
export default {
  ...
  methods: {
      ...
      this.$refs['deviceCategoryDetail'].validate((valid) => {
        if (valid) {
          ...
          func(Object.assign({}, this.deviceCategoryDetail)).then(res => {
            ...
            this.$parent.search({})
            this.$parent.getParents(res.data.typeCode)
            this.viewFlag = true
            this.deviceSaved = true
            this.getDeviceAttr()
            this.$message({
    ...
    // typeCode选择输入框的值发生改变
    typeCodeChange (typeCode) {
      this.deviceCategoryDetail.parentUuid = ''
      this.$parent.getParents(typeCode)
    }
  }
}
</script>
